...
module wb_interconnect_4slaves(
    ...
    // Slave 0 Interface (0xFFFF0000 - 0xFFFFFFFF, 64 kB)
    ...
    // Slave 1 Interface (0xFF010000 - 0xFF010FFF, 4 kB)
    ...
    // Slave 2 Interface (0x00000000 - 0x03FFFFFF, 64 MB SDRAM)
    ...
    parameter TARGET_SLAVE2 = 4'b0010;
    parameter TARGET_SLAVE3 = 4'b0011;
    ...
    // Master 0 targeting Slave0, Slave1, Slave2, or Slave3 based on address
    wire [3:0] m0_wbd_tid_i = (m0_wbd_adr_i >= 32'h00000000 && m0_wbd_adr_i <= 32'h03FFFFFF) ? TARGET_SLAVE2 :
                              (m0_wbd_adr_i >= 32'hFF010000 && m0_wbd_adr_i <= 32'hFF010FFF) ? TARGET_SLAVE1 :
                              (m0_wbd_adr_i >= 32'hFFFF0000 && m0_wbd_adr_i <= 32'hFFFFFFFF) ? TARGET_SLAVE0 :
                              TARGET_SLAVE3; // Default to Slave3 for other addresses
    
    // Master 1 targeting Slave0, Slave1, Slave2, or Slave3 based on address
    wire [3:0] m1_wbd_tid_i = (m1_wbd_adr_i >= 32'h00000000 && m1_wbd_adr_i <= 32'h03FFFFFF) ? TARGET_SLAVE2 :
                              (m1_wbd_adr_i >= 32'hFF010000 && m1_wbd_adr_i <= 32'hFF010FFF) ? TARGET_SLAVE1 :
    ...
    assign s2_wbd_dat_o = (s_bus_wr.wbd_tid == TARGET_SLAVE2) ? s_bus_wr.wbd_dat : 32'h0;
    assign s2_wbd_adr_o = (s_bus_wr.wbd_tid == TARGET_SLAVE2) ? s_bus_wr.wbd_adr : 32'h0;
    assign s2_wbd_sel_o = (s_bus_wr.wbd_tid == TARGET_SLAVE2) ? s_bus_wr.wbd_sel : 4'h0;
    assign s2_wbd_we_o  = (s_bus_wr.wbd_tid == TARGET_SLAVE2) ? s_bus_wr.wbd_we  : 1'b0;
    assign s2_wbd_cyc_o = (s_bus_wr.wbd_tid == TARGET_SLAVE2) ? s_bus_wr.wbd_cyc : 1'b0;
    assign s2_wbd_stb_o = (s_bus_wr.wbd_tid == TARGET_SLAVE2) ? s_bus_wr.wbd_stb : 1'b0;
    
    assign s3_wbd_dat_o = (s_bus_wr.wbd_tid == TARGET_SLAVE3) ? s_bus_wr.wbd_dat : 32'h0;
    assign s3_wbd_adr_o = (s_bus_wr.wbd_tid == TARGET_SLAVE3) ? s_bus_wr.wbd_adr : 32'h0;
    assign s3_wbd_sel_o = (s_bus_wr.wbd_tid == TARGET_SLAVE3) ? s_bus_wr.wbd_sel : 4'h0;
    assign s3_wbd_we_o  = (s_bus_wr.wbd_tid == TARGET_SLAVE3) ? s_bus_wr.wbd_we  : 1'b0;
    assign s3_wbd_cyc_o = (s_bus_wr.wbd_tid == TARGET_SLAVE3) ? s_bus_wr.wbd_cyc : 1'b0;
    assign s3_wbd_stb_o = (s_bus_wr.wbd_tid == TARGET_SLAVE3) ? s_bus_wr.wbd_stb : 1'b0;
    ...
    always_comb begin
        case(s_wbd_tid)
            ...
            TARGET_SLAVE2: s_bus_rd = s2_wb_rd;
            TARGET_SLAVE3: s_bus_rd = s3_wb_rd;
            default:       s_bus_rd = s0_wb_rd; // Default to Slave0
        endcase			
    end
    ...
    assign s2_wb_wr = (s_wbd_tid == TARGET_SLAVE2) ? s_bus_wr : 'h0;
    assign s3_wb_wr = (s_wbd_tid == TARGET_SLAVE3) ? s_bus_wr : 'h0;
